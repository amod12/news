...
class Matcher:
    ...
    def match_recommendation(self, indices, similarity):
        ...
        # names = [indices[x] for x in plc_lst]
    ...
    def matcher(self, title):
        ...
        tf_df = pd.DataFrame(
            tfidf_mat.toarray(), columns=tfidf.get_feature_names_out(), index=new_df["name"]
        )
        ...
        sim_df = pd.DataFrame(data=sim, index=new_df["name"], columns=new_df["name"])
